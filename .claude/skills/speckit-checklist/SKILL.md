...
description: Generate a custom, domain-specific quality checklist for the active feature (e.g. security, UX, API, performance, accessibility) - a 'unit test for the English' of the spec. Use when asked to create a checklist or define quality or acceptance gates for a feature. Requires plan.md.
---
...
2. Gate: `bash .specify/scripts/bash/check-prerequisites.sh --json`. If it exits non-zero or prints an ERROR about a missing `plan.md`, **STOP** and tell the user to run the **speckit-plan** skill first.
