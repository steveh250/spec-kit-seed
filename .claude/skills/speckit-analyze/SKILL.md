...
description: Run a non-destructive cross-artifact consistency and quality analysis across spec.md, plan.md, and tasks.md, reporting inconsistencies, duplication, ambiguities, coverage gaps, and constitution conflicts. Use when asked to analyze, audit, or check a feature's artifacts for consistency before implementing. Requires spec.md, plan.md, and tasks.md.
---
...
2. Gate on all three artifacts: `bash .specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks`. If it exits non-zero or prints an ERROR, **STOP** and run the named prerequisite skill (**speckit-specify** for a missing spec, **speckit-plan** for a missing plan, **speckit-tasks** for missing tasks). Do not produce partial analysis.
